...
10. **Slug-keyed URLs for tenants.** Frontend route segments use the slug (`/operator/tenants/acme-pharma/...`); UUID never appears in the URL bar. Slug → UUID resolution happens once at page load via `GET /v1/tenants/by-slug/{slug}`.
