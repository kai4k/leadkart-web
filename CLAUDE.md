...
2. **BFF via adapter-node.** SvelteKit's Node server is the auth-context owner. Three httpOnly cookies live at the SvelteKit ↔ browser boundary: the access JWT (`__Host-lk_access` in prod over HTTPS, `lk_access` in dev/test over HTTP — Chrome rejects `__Host-*` without Secure), the refresh token (`lk_refresh`, path-scoped to `/auth/refresh`), and the operator-scope context (`lk_op_tenant`, holds the currently-acted-on tenant identifier — never appears in the URL bar). SvelteKit talks server-to-server to the Go API with Bearer extracted from the access cookie and `X-Tenant-Id` extracted from the scope cookie; **the BFF is the sole writer of `X-Tenant-Id` — any value the browser supplies is ignored**. Browser never sees a token, never talks to Go directly. CSRF: double-submit-cookie via the non-httpOnly `lk_csrf` cookie + `X-CSRF-Token` header, compared with `crypto.timingSafeEqual`. SPA UX preserved via client-side routing on top of an SSR-rendered first paint. Full contract + flow diagrams in `docs/superpowers/specs/2026-05-20-bff-cookie-auth-adr.md`.
3. **TypeScript everywhere.** No `.js` source except generated types and config files.
4. **Svelte 5 runes only.** `$state`, `$props`, `$derived`, `$effect`, `$bindable`. Shared reactive logic lives in `*.svelte.ts` files (canonical extension) as **factory functions** — `export function createX(): X { ... }` returning an object whose reactive accessors are getters closing over local `$state` / `$derived`. NO classes (TS/React-OOP shape — Svelte 5 runes were designed for closures, not class fields). NO module-level `let foo = $state(...)` outside the documented `Toaster.svelte` singleton (silently breaks cross-module reactivity). Enforced by `check-no-class-stores` arch gate. Singletons via module export are fine when the singleton is the return value of a factory call (`export const theme = createTheme()`) — the proxy lives inside the closure, not at module scope. Error subclasses (`class FooError extends Error`) are exempt — they're type tags, not reactive stores.
...
9. **Identity-driven scope, single resource hierarchy** (per ADR 0038 draft — `docs/superpowers/specs/2026-05-18-adr-0038-draft-unified-endpoint-surface.md`). Frontend talks to `/v1/{resource}` paths only — NEVER `/v1/platform/{resource}` for tenant/users/roles/persons. Operator scope is set via `X-Tenant-Id` header (read-side) or scoped JWT from impersonation (write-side). `/v1/platform/*` paths are reserved for genuinely platform-wide operations (impersonation sessions, cross-tenant stats, platform-tenant convenience read).
10. **No sensitive identifiers in user-facing URLs.** Tenant slug, tenant UUID, person UUID and similar customer-identifying tokens stay out of the browser URL bar. The operator-scope flow uses a cookie, not a URL segment: clicking a tenant in `/operator/tenants` POSTs `{slug}` to `/api/operator/scope`, the BFF resolves slug → UUID server-side, sets the `lk_op_tenant` httpOnly cookie, and navigates to the slug-agnostic `/operator/scope/{profile,members,roles,activity,settings}` tree. The proxy auto-injects `X-Tenant-Id` from that cookie on every Go call. Exit context = `DELETE /api/operator/scope` → cookie cleared → back to `/operator/tenants`. API request paths under `/api/v1/*` may still carry resource IDs for the time being (visible only in DevTools, not the URL bar); migrating those to header-scoped paths is a backend follow-up. Pattern reference: Stripe Test Mode, Linear workspace switcher, GitHub Enterprise context switch.
11. **Mutations return the updated DTO.** Per ADR 0038, all backend mutations return 200 + the updated resource (not 204 empty). API client treats `response.data` as the canonical source of truth. NEVER write `await mutate(); await refetch();` — the response IS the refetch.
12. **Structured errors (RFC 9457 ProblemDetails).** API client throws typed error subclasses (`NetworkError`, `ValidationError`, `AuthError`, etc.) mapping from the response's `code` + `fields`. Components pattern-match on the subclass, not on the message string. Validation errors carry per-field error keys for inline form rendering.
...
| Class-based hooks (theme, use-form, use-url-filters, ...)                  | `src/lib/hooks/*.svelte.ts`                                                                                                             |
...
- Module-level `let foo = $state(...)` + function accessors (breaks cross-module reactivity — use factory pattern: `export function createX() { ... }` returning an object with getter accessors closing over local `$state`)
- `class X` in `*.svelte.ts` files (TS/React-OOP shape, not Svelte canon — `check-no-class-stores` arch gate enforces). Error subclasses (`class FooError extends Error`) are exempt.
