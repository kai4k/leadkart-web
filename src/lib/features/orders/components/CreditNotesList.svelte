...
<script lang="ts">
	...
	const sorted = $derived(
		[...creditNotes].sort((a, b) => +new Date(b.issued_at) - +new Date(a.issued_at))
	);
</script>
...
{:else}
	<div class="overflow-x-auto">
		<table class="w-full text-left text-sm" data-testid="credit-notes-list">
			<thead>
			...
			<tbody>
				{#each sorted as cn (cn.id)}
					<tr class="border-border border-b">
						...
						<td class="px-2 py-2 text-right tabular-nums">{formatMoney(cn.amount, currency)}</td>
